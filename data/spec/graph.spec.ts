...
describe("canFinish", () => {
    ...
    const trueScenarios: IScenario[] = [
        ...
        { numCourses: 4, prerequisites: [[1,0], [1,2], [1,3], [2,3]], output: true}
    ];

    const falseScenarios: IScenario[] = [
        { numCourses: 2, prerequisites: [[1,0], [0,1]], output: false },
        { numCourses: 20, output: false, prerequisites: [[0,10],[3,18],[5,5],[6,11],[11,14],[13,1],[15,1],[17,4]] }
    ];

    it ("Should return TRUE if the courses CAN be completed.", () => {
        trueScenarios.forEach(s => {
            expect(graph.canFinish(s.numCourses, s.prerequisites))
            .withContext(`Input: ${s.prerequisites}`)
            .toBe(s.output);
        });
    });
